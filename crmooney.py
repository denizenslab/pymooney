...
def crmooney_fromdb(api_key, api_secret, search_words, imagepath='', mooneypath='', dbname='flickr', search_tags=[], 
    ...
    for photo in photo_meta:
        if (counter % 50 == 0):
            print '\t\t {0}/{1} downloaded.'.format(counter, len(photo_meta))
        if size == 'original':
    ...
    print '{0} images are downloaded in {1}. \n Mooneys are in {2} and in the return variable'.format(len(imgs_mooney), imagepath, mooneypath)
    return imgs_mooney, photo_meta

def crmooney_frompath(filename, mooneypath='', resize=0, smooth_sigma=6, image_size=(400,400), threshold_method='global_otsu'):
...
if __name__ == '__main__':
    ...
    try:
        API_KEY, API_SECRET
        print 'Good news, you have API_KEY and API_SECRET defined. You are ready to go.'
        ...
        print 'You need an API_KEY and API_SECRET to continue. \n Check https://www.flickr.com/services/api'
